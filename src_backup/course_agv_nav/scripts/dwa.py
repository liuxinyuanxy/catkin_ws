...
def plan(self, state, config, goal, ob):
    ...
    for v in np.arange(v_min, v_max, config.v_reso):
        for omega in np.arange(omega_min, omega_max, config.yawrate_reso):
            dist = self.dist(state, config, ob, v, omega)
            if dist < 0:
                continue
            if v > math.sqrt(2 * dist * config.max_accel):
                continue
            if omega > math.sqrt(2 * dist * config.max_dyawrate):
                continue
            dist_cost = 1 / (dist + 1)
