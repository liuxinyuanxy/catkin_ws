...
def dist(state, config, ob, v, omega):
    # return 100
    x, y, theta, _, _ = state
    nearest_dist = np.inf
    for i in range(2):
...
def heading_cost(state, config, goal, v, omega):
    x, y, theta, _, _ = state
    dx = goal[0] - x
    dy = goal[1] - y
...
def speed_cost(config, v):
    return config.max_speed - v

def planning(state, config, goal, ob):
    ...
    for v in np.arange(v_min, v_max, config.v_reso):
        for omega in np.arange(omega_min, omega_max, config.yawrate_reso):
            dist_ = dist(state, config, ob, v, omega)
            if dist_ < 0:
                continue
            ...
            if omega > math.sqrt(2 * dist_ * config.max_dyawrate):
                continue
            dist_cost = 1 / (dist_ + 1)
            cost = heading_cost(state, config, goal, v, omega) * config.to_goal_cost_gain + speed_cost(
